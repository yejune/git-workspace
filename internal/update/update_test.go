...
func TestNewUpdater(t *testing.T) {
	t.Run("creates updater with defaults", func(t *testing.T) {
		...
		if updater.RepoName != "git-subclone" {
			t.Errorf("expected RepoName 'git-subclone', got %s", updater.RepoName)
		}
...
func TestGetAssetName(t *testing.T) {
	...
	expectedName := fmt.Sprintf("git-subclone-%s-%s", runtime.GOOS, runtime.GOARCH)
		...
		mockClient := &MockHTTPClient{
			DoFunc: func(req *http.Request) (*http.Response, error) {
				body := `{
					"tag_name": "v2.0.0",
					"assets": [
						{"name": "git-subclone-darwin-arm64", "browser_download_url": "https://example.com/download"}
					]
				}`
...
func TestUpdate(t *testing.T) {
	t.Run("successful update", func(t *testing.T) {
		...
		tempDir := t.TempDir()
		execPath := filepath.Join(tempDir, "git-subclone")
	...
	t.Run("no binary for platform", func(t *testing.T) {
		...
		release := &GitHubRelease{
			TagName: "v2.0.0",
			Assets: []Asset{
				{Name: "git-subclone-windows-amd64", BrowserDownloadURL: "https://example.com/download"},
			},
		}
...
func TestGetLatestRelease(t *testing.T) {
	...
	t.Run("verifies request headers", func(t *testing.T) {
		...
		if capturedReq.Header.Get("User-Agent") != "git-subclone-updater" {
...
func TestGetAssetNameArchitectures(t *testing.T) {
	...
	if !strings.HasPrefix(assetName, "git-subclone-") {
		t.Errorf("expected prefix 'git-subclone-', got %s", assetName)
	}
